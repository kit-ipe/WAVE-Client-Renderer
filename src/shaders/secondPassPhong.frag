...
vec3 getNormal(vec3 at)
{
    ...
    <% for(var i=0; i < maxTexturesNumber; i++) { %>
        if( iTexLowerIndex == <%=i%> )
        {
            texpos1.x = dx1+((at.x - 1.0/256.0 )/uSlicesOverX);
            ...
            texpos1.y = dy1+((at.y-1.0/256.0) /uSlicesOverY);
        ...
        if( iTexUpperIndex == <%=i%> ) {
            texpos1.x = dx2+((at.x - 1.0/256.0)/uSlicesOverX);
            ...
            texpos1.y = dy2+((at.y-1.0/256.0) /uSlicesOverY);
    ...
    x0 = ((weight * (x10 - x00)) + x00)*255.0;
    x1 = ((weight * (x11 - x01)) + x01)*255.0;
    
    y0 = ((weight * (y10 - y00)) + y00)*255.0;
    y1 = ((weight * (y11 - y01)) + y01)*255.0;
    
    weight_z0 = (at.z - (1.0/256.0)) - floor(at.z);
    weight_z1 = (at.z + (1.0/256.0)) - floor(at.z);
    z0 = ((weight_z0 * (z11 - z00)) + z00)*255.0;
    z1 = ((weight_z1 * (z11 - z00)) + z00)*255.0;
